...
import org.jhonrain.common.utils.ShiroUtils;
...
public class SysLoginController {
  ...
  public SysResp login(String username, String password, String captcha) throws IOException {
    String kaptcha = ShiroUtils.getKaptcha(Constants.KAPTCHA_SESSION_KEY);
    if (!captcha.equalsIgnoreCase(kaptcha)) {
      return SysResp.error(100001, "验证码不正确");
    }
    ...
    if (user == null || !user.getPassword().equals(new Sha256Hash(password, user.getSalt()).toHex())) {
      return SysResp.error(100002, "账号或密码不正确");
    }

    //账号锁定
    if (user.getStatus() == 0) {
      return SysResp.error(100003, "账号已被锁定,请联系管理员");
    }
